    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-8 bg-black/90 backdrop-blur-md">
        <motion.div 
          ...
          className="w-full max-w-5xl h-[85vh] bg-[#0a0a0a] border border-white/10 rounded-[3rem] overflow-hidden flex flex-col shadow-[0_50px_100px_rgba(0,0,0,0.8)]"
        >
          {/* Header */}
          <div className="p-8 border-b border-white/5 flex items-center justify-between bg-white/[0.02]">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                   ...
                   <p className="text-[10px] text-white/40 uppercase font-black tracking-[0.2em]">RESEARCHAI_SYNTHESIS_NODE :: {activeArtifact.timestamp}</p>
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <button className="flex items-center gap-2 px-6 py-3 bg-accent text-black rounded-xl text-[10px] font-black uppercase tracking-widest hover:opacity-90 transition-all shadow-lg">
                <Download size={14} /> Export
              </button>
              <div className="w-[1px] h-8 bg-white/10 mx-2" />
              <button 
                onClick={clearStudioArtifacts}
                className="w-12 h-12 flex items-center justify-center bg-white/5 text-white/20 hover:text-white rounded-full transition-all"
              >
                <X size={20} />
              </button>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-12 bg-[#080808]">
             <div className="max-w-3xl mx-auto">
                <pre className="text-white/80 font-medium leading-relaxed whitespace-pre-wrap font-sans text-lg">
                ...
                {activeArtifact.type === 'audio' && (
                  <div className="mt-12 p-12 rounded-[2rem] bg-accent/5 border border-accent/10 text-center">
                    ...
                    <p className="text-sm text-white/40">Ready for PhD-level voice synthesis via ElevenLabs / Local TTS.</p>
                  </div>
                )}
             </div>
          </div>
