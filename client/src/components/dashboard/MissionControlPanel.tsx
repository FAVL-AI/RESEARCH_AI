...
import { Play, Square, Clock, Target, List, CheckCircle } from "lucide-react";
...
export const MissionControlPanel = () => {
  ...
  return (
    <div className="p-6 bg-black/40 backdrop-blur-xl border border-white/10 rounded-3xl h-full flex flex-col gap-6">
      ...
      {!mission ? (
        <div className="space-y-4">
          <div>
            <label className="text-[10px] uppercase font-black tracking-widest text-white/40 mb-2 block">Research Objective</label>
            <textarea
              value={goal}
              onChange={(e) => setGoal(e.target.value)}
              placeholder="e.g. Map the evolution of transformer architecture..."
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-white text-sm focus:border-cyan-500/50 outline-none h-32 resize-none transition-all"
            />
          </div>
          
          <div className="flex gap-4">
            <div className="flex-1">
              <label className="text-[10px] uppercase font-black tracking-widest text-white/40 mb-2 block">Duration (Hours)</label>
              <input 
                type="number" 
                value={duration} 
                onChange={(e) => setDuration(parseInt(e.target.value))}
                className="w-full bg-white/5 border border-white/10 rounded-2xl p-3 text-white outline-none focus:border-cyan-500/50"
              />
            </div>
          </div>
        ...
        <div className="flex-1 flex flex-col gap-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] uppercase font-black text-white/40 tracking-widest">Elapsed</span>
              </div>
            ...
            <div className="p-4 bg-white/5 border border-white/10 rounded-2xl">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] uppercase font-black text-white/40 tracking-widest">Insights</span>
              </div>
              <div className="text-xl font-mono text-white">
          ...
          <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-white/10">
            <label className="text-[10px] uppercase font-black tracking-widest text-white/40 sticky top-0 bg-black/5 py-1">Recent Checkpoints</label>
            {mission.checkpoints.slice().reverse().map((cp: any, i: number) => (
              <div key={i} className="flex gap-3 items-start animate-in fade-in slide-in-from-left-4 duration-500">
                ...
                <div className="space-y-1">
                  <div className="text-[11px] text-white/80 leading-relaxed font-medium">{cp.summary}</div>
                  <div className="text-[9px] text-white/40 font-mono italic">{new Date(cp.timestamp).toLocaleTimeString()}</div>
                </div>
              </div>
            ))}
          </div>
