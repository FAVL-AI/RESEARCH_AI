...
export const SupervisorPanel = () => {
    ...
    if (!pending) return (
        <div className="p-6 bg-white/[0.02] border border-white/5 rounded-3xl opacity-40">
            <h3 className="text-[10px] font-black tracking-widest uppercase text-white/40 mb-4">Supervisor Protocol</h3>
            <p className="text-[9px] text-white/20 uppercase tracking-widest text-center py-8">No Pending Decisions</p>
        </div>
    );
    ...
    return (
        <div className="p-6 bg-white/[0.02] border border-accent/20 rounded-3xl backdrop-blur-3xl animate-in fade-in slide-in-from-bottom-4">
            <div className="flex items-center justify-between mb-6">
                ...
                <span className="text-[8px] font-bold text-white/20 uppercase">ID: {missionId}</span>
            </div>
            ...
            <div className="space-y-4">
                ...
                <textarea 
                    ...
                    className="w-full h-24 bg-black/60 border border-white/10 rounded-2xl p-4 text-[10px] text-white/80 placeholder:text-white/20 focus:outline-none focus:border-accent/40 transition-all resize-none"
                />

                <div className="grid grid-cols-3 gap-2">
                    <button 
                        onClick={() => handleDecision('approve')}
                        className="flex flex-col items-center justify-center p-3 rounded-2xl bg-accent/10 border border-accent/20 hover:bg-accent/20 transition-all group"
                    >
